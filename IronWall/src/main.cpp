...
int main() {
	std::cout << "Hello From User Land\n";

	const DWORD pid = get_process_id(L"Magnify.exe");
	...
	const HANDLE driver = CreateFile(L"\\\\.\\IronWall", GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);

	if (driver::attach_to_process(driver, pid)) {
		std::cout << "Attachment successfuly!\n";
	}
